...
/datum/component/scope/proc/zoom(mob/user)
	...
	if(user.is_holding(parent))
		RegisterSignal(user, list(COMSIG_MOB_SWAP_HANDS, COMSIG_PARENT_QDELETING), PROC_REF(stop_zooming))
		...
		RegisterSignal(user, capacity_signals, PROC_REF(on_incapacitated))
