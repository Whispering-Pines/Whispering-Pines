	...
	if(world.time > last_eat + 50)
		...
		if(istype(AM, /mob/living/carbon/human))
			...
			if(L.wear_armor)
				...
				var/turf/target = get_ranged_target_turf(src, pick(GLOB.alldirs), 2)
				L.dropItemToGround(citem)
				citem.throw_at(target, 2, 2)
			if(L.wear_pants)
		...
		if(world.time > last_eat + 30)
			...
			if(!is_fucking)
				...
				if(L.getorganslot(ORGAN_SLOT_PENIS))
					...
					addtimer(CALLBACK(src, PROC_REF(start_obj_sex), L, SEX_SPEED_EXTREME, SEX_FORCE_HIGH, TRUE, ORGAN_SLOT_PENIS, /datum/reagent/consumable/cum/plant, 15), 0.1 SECONDS, TIMER_STOPPABLE)
				if(L.getorganslot(ORGAN_SLOT_VAGINA))
					...
					addtimer(CALLBACK(src, PROC_REF(start_obj_sex), L, SEX_SPEED_EXTREME, SEX_FORCE_HIGH, TRUE, ORGAN_SLOT_VAGINA, /datum/reagent/consumable/cum/plant, 15), 0.2 SECONDS, TIMER_STOPPABLE)
				if(L.getorganslot(ORGAN_SLOT_GUTS) && C.gender == FEMALE) //no good way of knowing their prefs so imma just make it non homo.
					...
					addtimer(CALLBACK(src, PROC_REF(start_obj_sex), L, SEX_SPEED_EXTREME, SEX_FORCE_HIGH, TRUE, ORGAN_SLOT_GUTS, /datum/reagent/consumable/cum/plant, 15), 0.3 SECONDS, TIMER_STOPPABLE)
				if(C.gender == FEMALE)
