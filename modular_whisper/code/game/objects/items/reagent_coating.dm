...
/obj/item/weapon/Initialize(...)
	...
	if(sharpness != IS_BLUNT && !uncoatable)
		...
		reagent_apply_amt = max((5/w_class), 1) //smaller weapons will apply more poison at once.
...
/obj/item/weapon/proc/__apply_reagents(obj/item/I, mob/living/user, mob/living/H)
...
/obj/item/weapon/proc/apply_reagents(obj/item/I, mob/living/user, mob/living/H)
	if(!H || user == H)
		return
	if(reagents.total_volume)
		reagents.trans_to(user, reagent_apply_amt, 1, no_react = FALSE)
		user.visible_message(span_green("[user] shudders!"),span_boldgreen("I feel a burning pain spreading through my veins!"))
		log_attack("[user] was struck with [english_list(reagents.reagent_list)] using a poisoned [I.name] by [H].")

/obj/item/weapon/proc/__clean_react()
	SIGNAL_HANDLER
...
/obj/item/weapon/proc/clean_react()
	if(reagents?.total_volume)
		reagents.remove_all(reagents.maximum_volume) //buh bye reagents water washes it

/obj/item/weapon/attackby(obj/item/reagent_containers/I, mob/living/user, params)
	if(reagents)
