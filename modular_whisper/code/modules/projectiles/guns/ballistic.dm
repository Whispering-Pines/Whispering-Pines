...
/obj/item/gun/ballistic
	...
	pickup_sound = 'modular_whisper/sound/items/handling/gun/gun_pick_up.ogg'
	drop_sound = 'modular_whisper/sound/items/handling/gun/gun_drop.ogg'
	vary_fire_sound = TRUE

	///sound when inserting magazine
	var/load_sound = 'modular_whisper/sound/items/weapons/gun/general/magazine_insert_full.ogg'
	///sound when inserting an empty magazine
	var/load_empty_sound = 'modular_whisper/sound/items/weapons/gun/general/magazine_insert_empty.ogg'
	///volume of loading sound
	var/load_sound_volume = 40
	///whether loading sound should vary
	var/load_sound_vary = TRUE
	///sound of racking
	var/rack_sound = 'modular_whisper/sound/items/weapons/gun/general/bolt_rack.ogg'
	///volume of racking
	var/rack_sound_volume = 60
	///whether racking sound should vary
	var/rack_sound_vary = TRUE
	///sound of when the bolt is locked back manually
	var/lock_back_sound = 'modular_whisper/sound/items/weapons/gun/general/slide_lock_1.ogg'
	///volume of lock back
	var/lock_back_sound_volume = 60
	///whether lock back varies
	var/lock_back_sound_vary = TRUE
	///Sound of ejecting a magazine
	var/eject_sound = 'modular_whisper/sound/items/weapons/gun/general/magazine_remove_full.ogg'
	///sound of ejecting an empty magazine
	var/eject_empty_sound = 'modular_whisper/sound/items/weapons/gun/general/magazine_remove_empty.ogg'
	///volume of ejecting a magazine
	var/eject_sound_volume = 40
	///whether eject sound should vary
	var/eject_sound_vary = TRUE
	///sound of dropping the bolt or releasing a slide
	var/bolt_drop_sound = 'modular_whisper/sound/items/weapons/gun/general/bolt_drop.ogg'
	///volume of bolt drop/slide release
	var/bolt_drop_sound_volume = 60
	///empty alarm sound (if enabled)
	var/empty_alarm_sound = 'modular_whisper/sound/items/weapons/gun/general/empty_alarm.ogg'
...
/obj/item/gun/ballistic/proc/burst_select()
	...
	playsound(user, 'modular_whisper/sound/items/weapons/empty.ogg', 100, TRUE)
	update_appearance()
				...
				if(ishuman(loc))
					...
					switch(caught_casing)
						...
						if(CASING_CATCH_FAILED_SPICY)
							...
							wielder.apply_damage(2, BURN, affecting)
						if(CASING_CATCH_FAILED_CLUMSY)
							...
							if(!(world.time >= casing.shot_timestamp + CASING_HOT_DELAY))
								var/obj/item/bodypart/affecting = wielder.get_inactive_hand()
								to_chat(wielder, span_warning("As if to add insult to injury, \the [casing] lands in the perfect way... to burn your [affecting.name]."))
						...
						if(CASING_CATCH_SUCCESSFUL_OUCH)
							...
							casing.update_appearance()
							var/obj/item/bodypart/affecting = wielder.get_inactive_hand()
							to_chat(wielder, span_notice("You reach out and catch \the [casing] as it ejects from [src]. Awesome. Your [affecting.name] hurts, though."))
							wielder.apply_damage(4, BURN, affecting)
				if(hitting_ground)
...
/obj/item/gun/ballistic/proc/can_catch_casing(obj/item/ammo_casing/casing, mob/living/carbon/human/wielder)
	if(!wielder.in_throw_mode) // if they're not in throw mode, don't bother
