...
/obj/item/ammo_casing/proc/ready_proj(atom/target, mob/living/user, quiet, zone_override = "", atom/fired_from)
	...
	loaded_projectile.hit_prone_targets = user.cmode
	if (zone_override)
	...
	if(isgun(fired_from))
		...
		loaded_projectile.damage *= gun.damfactor * integrity_mult
		loaded_projectile.stamina *= gun.damfactor * integrity_mult

		loaded_projectile.speed *= gun.projectile_speed_multiplier * integrity_mult

		loaded_projectile.embedchance += gun.embedchance
		loaded_projectile.embedchance *= loaded_projectile.embedchance >= 0 ? 1 : 2 - integrity_mult
		loaded_projectile.exposed_embedchance += gun.embedchance
		loaded_projectile.exposed_embedchance *= loaded_projectile.exposed_embedchance >= 0 ? 1 : 2 - integrity_mult
