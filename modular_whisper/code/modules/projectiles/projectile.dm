...
/obj/projectile
	name = "projectile"
	icon = 'modular_whisper/icons/obj/weapons/guns/projectiles.dmi'
	icon_state = "bullet"
	density = FALSE
	anchored = TRUE
	mouse_opacity = MOUSE_OPACITY_TRANSPARENT
	movement_type = FLYING
	embedchance = -100 // can't wound by default
	generic_canpass = FALSE
	blocks_emissive = EMISSIVE_BLOCK_GENERIC
	layer = MOB_LAYER
	/// The sound this plays on impact.
	var/hitsound = 'modular_whisper/sound/items/weapons/pierce.ogg'
	...
	/// How much we want to drop both embedchance and exposed_embedchance (to a minimum of 0 for the latter) per tile, for falloff purposes
...
/obj/projectile/proc/reduce_range()
	...
	if(wound_falloff_tile && embedchance != -100)
		embedchance += wound_falloff_tile
...
/obj/projectile/proc/check_ricochet_flag(atom/target)
	if((armor_flag in list(ENERGY, LASER)) && (target.flags_ricochet & RICOCHET_SHINY))
		return TRUE
	if((armor_flag in list(BOMB, "piercing")) && (target.flags_ricochet & RICOCHET_HARD))
		return TRUE
	return FALSE
