...
/datum/status_effect/tased/proc/set_taser(datum/new_taser)
	taser = new_taser
	RegisterSignal(taser, list(COMSIG_PARENT_QDELETING, COMSIG_ITEM_DROPPED, COMSIG_ITEM_EQUIPPED), PROC_REF(end_tase))
...
/datum/status_effect/tased/proc/set_firer(atom/new_firer)
	...
	if(taser != firer) // Turrets, notably, are both
		RegisterSignal(firer, COMSIG_PARENT_QDELETING, PROC_REF(end_tase))
	...
	RegisterSignal(tase_line, COMSIG_PARENT_QDELETING, PROC_REF(end_tase))
