...
//todo make this digital
/obj/structure/fake_machine/noticeboard
	name = "Notice Board"
	desc = "A large wooden notice board, carrying postings from all across the world. A crow perch sits atop it."
	icon = 'modular_azure/icons/misc/64x64.dmi'
...
/obj/structure/fake_machine/boardbarrier //Blocks sprite locations
	name = ""
	desc = "A large wooden notice board, carrying postings from all across the world. A crow perch sits atop it."
...
/obj/structure/fake_machine/noticeboard/Initialize()
	. = ..()
...
/obj/structure/fake_machine/noticeboard/examine(mob/living/carbon/human/user)
	. = ..()
	if(!ishuman(user))
		return
...
/obj/structure/fake_machine/noticeboard/update_icon()
	. = ..()
...
/obj/structure/fake_machine/noticeboard/Topic(href, href_list)
	. = ..()
...
/obj/structure/fake_machine/noticeboard/attack_hand(mob/living/carbon/human/user)
	if(!ishuman(user))
		return
...
/obj/structure/fake_machine/noticeboard/proc/premium_post(mob/living/carbon/human/guy)
	if(guy.has_status_effect(/datum/status_effect/postcooldown))
	...
	guy.apply_status_effect(/datum/status_effect/postcooldown)
	message_admins("[ADMIN_LOOKUPFLW(guy)] has made a notice board post. The message was: [inputmessage]")
	for(var/obj/structure/fake_machine/noticeboard/board in SSroguemachine.noticeboards)
		if(board != src)
...
/obj/structure/fake_machine/noticeboard/proc/make_post(mob/living/carbon/human/guy)
	if(guy.has_status_effect(/datum/status_effect/postcooldown))
		to_chat(guy, span_warning("I must wait a time until my next posting..."))
		return
	...
	if(length(inputtitle) > 50)
		to_chat(guy, span_warning("Too long! You shall surely overburden the crow with this novel!"))
		return
	var/inputmessage = stripped_multiline_input(guy, "What shall I write for this posting?", "NOTICEBOARD", no_trim=TRUE)
	if(inputmessage)
		if(length(inputmessage) > 2000)
...
/obj/structure/fake_machine/noticeboard/proc/remove_post(mob/living/carbon/human/guy)
	...
	for(var/obj/structure/fake_machine/noticeboard/board in SSroguemachine.noticeboards)
		...
		if(board != src)
			playsound(board, 'sound/ambience/noises/birds (7).ogg', 50, FALSE, -1)
			board.visible_message(span_smallred("A crow lands, removing an old posting!"))

/obj/structure/fake_machine/noticeboard/proc/authority_removepost(mob/living/carbon/human/guy)
...
/datum/status_effect/postcooldown
	id = "postcooldown"
	duration = 5 MINUTES
	alert_type = /atom/movable/screen/alert/status_effect/postcooldown

/atom/movable/screen/alert/status_effect/postcooldown
	name = "Recent messenger"
	icon = 'modular_whisper/licensed_eaglephntm/icons/mob/screen_alert.dmi'
	icon_state = "mail_cd"
	desc = "I'll have to wait a bit before making another posting."

/obj/structure/fake_machine/noticeboard/proc/consult_quests(mob/user)
...
/obj/structure/fake_machine/noticeboard/proc/find_quest_landmark(difficulty, type)
...
/obj/structure/fake_machine/noticeboard/proc/turn_in_quest(mob/user)
	var/reward = 0
...
/obj/structure/fake_machine/noticeboard/proc/cash_in(reward, original_reward)
...
/obj/structure/fake_machine/noticeboard/proc/abandon_quest(mob/user)
...
/obj/structure/fake_machine/noticeboard/proc/print_quests(mob/user)
