	...
	switch(browser_input_list(user, "What to do?", null, useoptions))
		...
		if("Steal the Sun")
			if(!can_steal_sun(lord_datum))
				return
			if(browser_alert(user, "Force Phantom Kingdom into Night?<BR>Cost:[SUN_STEAL_COST]","",DEFAULT_INPUT_CHOICES) != CHOICE_YES)
				return
			if(!do_after(user, 10 SECONDS, src))
				return
			if(!can_steal_sun(lord_datum))
			...
			for(var/mob/living/carbon/human/astrater as anything in GLOB.human_list)
				if(!istype(astrater.patron, /datum/patron/divine/solaria))
					continue
