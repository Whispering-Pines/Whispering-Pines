...
/mob/dead/observer/proc/reenter_corpse(forced = FALSE)
	...
	if(istype(src, /mob/dead/observer/profane))
		to_chat(src, "<span class='warning'>My spirit has been snatched away by Sinius!</span>")
		return
...
/mob/dead/observer/proc/horde_respawn()
	...
	if(alert(src, "You have been summoned to destroy Phantom Kingdom!", "Join the Horde", "Yes", "No") == "Yes")
