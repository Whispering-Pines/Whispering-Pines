...
/datum/species/harpy
	...
	Their serene songs and blissful music can be heard echoing far below, guiding travelers and thieves both to respite... or treasure. For as lifted into grace as they might be, these 'magpies' earn such a nickname from Dismasan greed and love for anything that shines. \
	...
	inherent_traits = list(TRAIT_HOLLOWBONES, TRAIT_AMAZING_BACK)

	specstats_m = list(STATKEY_STR = -1, STATKEY_PER = 2, STATKEY_INT = 0, STATKEY_CON = -3, STATKEY_END = 1, STATKEY_SPD = 2, STATKEY_LCK = 0)
	specstats_f = list(STATKEY_STR = -1, STATKEY_PER = 2, STATKEY_INT = 0, STATKEY_CON = -3, STATKEY_END = 1, STATKEY_SPD = 2, STATKEY_LCK = 0)
