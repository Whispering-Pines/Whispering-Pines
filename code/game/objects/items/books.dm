...
/obj/item/book/secret/ledger/proc/generate_html(mob/user)
	...
	for(var/datum/supply_pack/pack in types)
		...
		if(should_show)
			...
			html += "<a class='item-link [availability_class] [stock_class] [reputation_class]' data-in-stock='[is_available ? "true" : "false"]' href='byond://?src=\ref[src];action=[action_url]&item=\ref[pack]&quantity=1' style='[display_style]'>[pack.name] ([pack.cost] casings)[availability_text][reputation_cost_text]</a>"
	...
	if(length(cart) > 0)
		...
		for(var/datum/supply_pack/pack in cart)
			...
			html += "</form>"
			html += "</td>"
			html += "<td>[item_cost] casings[is_reputation_purchase ? " + [reputation_cost] rep" : ""]</td>"
			html += "<td><span class='remove-item' onclick=\"location.href='byond://?src=\ref[src];action=remove_from_cart&item=\ref[pack]'\">✖</span></td>"
			html += "</tr>"

		html += "<tr><td colspan='2'><strong>Total:</strong></td><td colspan='2'><strong>[total_cost] casings"
