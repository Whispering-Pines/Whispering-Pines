...
/obj/structure/fake_machine/mail/attack_hand_secondary(mob/user, params)
	...
	if(!coin_loaded)
		...
		return
	var/send2place = input(user, "Where to? (Person or #number)", "WHISPERING PINES", null)
	if(!send2place)
		return
	var/sentfrom = input(user, "Who is this letter from?", "WHISPERING PINES", null)
	if(!sentfrom)
		sentfrom = "Anonymous"
	var/t = stripped_multiline_input("Write Your Letter", "WHISPERING PINES", no_trim=TRUE)
	if(t)
		if(length(t) > 2000)
...
/obj/structure/fake_machine/mail/attackby(obj/item/P, mob/user, params)
	...
	if(istype(P, /obj/item/paper))
		...
		if(alert(user, "Send Mail?",,"YES","NO") == "YES")
			var/send2place = input(user, "Where to? (Person or #number)", "WHISPERING PINES", null)
			var/sentfrom = input(user, "Who is this from?", "WHISPERING PINES", null)
			if(!sentfrom)
				sentfrom = "Anonymous"
			if(findtext(send2place, "#"))
				var/box2find = text2num(copytext(send2place, findtext(send2place, "#")+1))
...
/obj/structure/fake_machine/mail/proc/show_inquisitor_shop(mob/living/carbon/human/user)
	...
	var/list/items = list(
		// Weapons
		"Flintlock pistol Pistol- 4 Lead Balls- Powder Flask  (8)" = list(
