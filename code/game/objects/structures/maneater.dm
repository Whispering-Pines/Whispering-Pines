...
/obj/structure/flora/grass/maneater/real/user_unbuckle_mob(mob/living/M, mob/user)
	...
	if(isliving(user))
		...
		if(istype(src, /obj/structure/flora/grass/maneater/real/juvenile)) //Easier to escape juvenile
...
/obj/item/maneaterseed
	...
	icon = 'modular_azure/icons/mob/monster/maneater.dmi'
...
/obj/structure/flora/grass/maneater/real/juvenile/process()
	if(!has_buckled_mobs())
		if(world.time > last_eat + 50)
			var/list/around = view(1, src)
			for(var/mob/living/M in around)
...
/obj/structure/flora/grass/maneater/real/juvenile/Destroy()
	STOP_PROCESSING(SSobj, src)
	return ..()
